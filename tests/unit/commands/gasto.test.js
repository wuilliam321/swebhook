...
describe('Command: Gasto', () => {
    ...
    test('locks the History spreadsheet while defaulting the store label to Ambas', async () => {
        context.userCommand = '/gastos_history';
        await gastoCommand.execute(context);
        expect(context.chatStates[123]).toMatchObject({ store: 'Ambas', targetStore: 'History' });
    });
